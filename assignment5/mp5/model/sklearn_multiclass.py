...
def sklearn_multiclass_prediction(mode, X_train, y_train, X_test):
    ...
    '''
    if(mode == 'ovr'):
        classifier = multiclass.OneVsRestClassifier(svm.LinearSVC(random_state=12345))
    elif(mode == 'ovo'):
        classifier = multiclass.OneVsOneClassifier(svm.LinearSVC(random_state=12345))
    elif(mode == 'crammer'):
        classifier = svm.LinearSVC(multi_class='crammer_singer', random_state=12345)
    else:
