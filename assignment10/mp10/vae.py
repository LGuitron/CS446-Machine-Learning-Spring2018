...
class VariationalAutoencoder(object):
    ...
    def _sample_z(self, z_mean, z_log_var):
        ...
        z = None
        ####### Implementation Here ######
        z_std_dev = tf.sqrt(tf.exp(z_log_var))
        #print("RAND: " , self.session.run(epsilon))
        
        
        z = z_mean + z_std_dev * tf.random_normal(tf.shape(z_std_dev))
        return z
    ...
    def _encoder(self, x):
        ...
        z_log_var = None
        ####### Implementation Here ######
        hidden1 = tf.layers.dense(x, 100 , activation = tf.nn.softplus)
        hidden2 = tf.layers.dense(hidden1, 50 , activation = tf.nn.softplus)
        
        z_mean = tf.layers.dense(hidden2, self._nlatent)
        z_log_var = tf.layers.dense(hidden2, self._nlatent)
    ...
    def _decoder(self, z):
        ...
        f = None
        ####### Implementation Here ######
        hidden1 = tf.layers.dense(z, 50 , activation = tf.nn.softplus)
        hidden2 = tf.layers.dense(hidden1, 100 , activation = tf.nn.softplus)
        
        f = tf.layers.dense(hidden2, self._ndims, activation = tf.nn.sigmoid)
    ...
    def _latent_loss(self, z_mean, z_log_var):
        ...
        latent_loss = tf.distributions.kl_divergence(tf.distributions.Normal(z_mean,z_std_dev), tf.distributions.Normal(tf.zeros(tf.shape(z_mean)),tf.ones(tf.shape(z_std_dev))))
