...
class Neural_Network():
    ...
    def forward(self, X):
        ...
        m1 = np.matmul(X, self.U)
        self.z = self.e + m1
        self.b = self.sigmoid(self.z)
        
        # Hiddent layer to output layer
        m2 = np.matmul(self.b, self.W)
        self.h = self.f + m2
    ...
    def error_at_layer2(self, d_o, o):
        ...
        '''
        delta_k = d_o*self.sigmoidPrime(self.sigmoid(self.h))
        return delta_k
    ...
    def error_at_layer1(self, delta_k, W, b):
        ...
        delta_j = np.matmul(self.W, np.transpose(delta_k))
        delta_j = delta_j * np.transpose(self.sigmoidPrime(self.sigmoid(self.z)))
        delta_j = np.transpose(delta_j)
        return delta_j
...
def main():
    ...
    a = np.random.randint(1, high=10, size=[batchSize,2], dtype='l')
