...
def preprocess_data(data, process_method='default'):
    ...
    if process_method == 'raw':
        ...
        data['image'] = data['image']/255
        tempImage = skimage.color.rgb2grey(data['image'])
        tempImage = skimage.color.grey2rgb(tempImage)
        data['image'] = np.absolute(data['image'] - tempImage)
        data = remove_data_mean(data)
        data['image'] = data['image'].reshape(data['image'].shape[0], -1)
...
def compute_image_mean(data):
    ...
    """
    return np.mean(data['image'], axis=0)
