...
class LinearMRF(object):
    ...
    def calculate_unary_potentials(self, unary_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        unary_potentials = tf.multiply(self.unary_weight, tf.convert_to_tensor(unary_features, dtype = tf.float32))
    ...
    def calculate_pairwise_potentials(self, pairwise_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        pairwise_potentials = tf.multiply(self.pairwise_weight, tf.convert_to_tensor(pairwise_features, dtype = tf.float32))
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        obj = tf.zeros(1, tf.float32)
        
        
        for i in range(len(unary_beliefs)):
            # Noisy image score
            obj += tf.reduce_sum(tf.multiply(tf.cast(unary_beliefs[i], tf.float32), tf.cast(unary_potentials[i], tf.float32)))
            obj += tf.reduce_sum(tf.multiply(tf.cast(pair_beliefs[i], tf.float32), tf.cast(pairwise_potentials, tf.float32)))
            
            # True image score
            correct_unary_potentials = self.calculate_unary_potentials(img_features)
            correct_pairwise_beliefs = self.get_pairwise_beliefs(img_features)
            correct_pairwise_potentials = self.calculate_pairwise_potentials(correct_pairwise_beliefs)
            obj -= tf.reduce_sum(tf.multiply(tf.convert_to_tensor(img_features, dtype=tf.float32), tf.cast(correct_unary_potentials, tf.float32)))
            obj -= tf.reduce_sum(tf.multiply(tf.convert_to_tensor(correct_pairwise_beliefs, dtype=tf.float32), tf.cast(correct_pairwise_potentials, tf.float32)))
        return obj
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        '''
        copy_unary_beliefs = unary_beliefs.copy()
        order = np.random.permutation(self.width*self.height)
        
        for i in range(self.width*self.height):
            asg_0 = self.calculate_local_score(order[i], 0, copy_unary_beliefs, unary_pots, pairwise_pots)
            asg_1 = self.calculate_local_score(order[i], 1, copy_unary_beliefs, unary_pots, pairwise_pots)
            
            #Assign new belief for this pixel
            if(asg_0>asg_1):
                copy_unary_beliefs[order[i]][0] = 1
                copy_unary_beliefs[order[i]][1] = 0
            else:
                copy_unary_beliefs[order[i]][1] = 1
                copy_unary_beliefs[order[i]][0] = 0
        #######################
        #IMPLEMENT THIS METHOD#
        #######################
        return copy_unary_beliefs
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        # Pairwise score
        for j in range(len(self.neighbors[node])):
            neighbor = self.neighbors[node][j]
            score += unary_potentials[neighbor][assignment]
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        similarity = new_unary_beliefs==old_unary_beliefs
        similarity = np.sum(similarity)/(self.height*self.width*2)
        return similarity > (1-convergence_margin)
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        for i in range(len(unary_beliefs)):
            ...
            for j in range(len(neighbors)):
                ...
                # [0, 1][1, 0] = 2
                # [0, 1][0, 1] = 3
                # index to be 1 in the pairwise beliefs for the current pair
                active_index = 2*int(unary_beliefs[i][1]) + int(unary_beliefs[neighbors[j]][1])
                pair_index = int(self.pair_inds[(i,neighbors[j])])
                result[pair_index, active_index] = 1
        return result
